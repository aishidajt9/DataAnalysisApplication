...
この正規方程式は，残差ベクトル$\mathbf{e}=\mathbf{y}-\mathbf{X}\mathbf{b}$を用いると，右辺を移項して次のように書き直せる． $$
\begin{align}
    \mathbf{X}'\mathbf{y} - \mathbf{X}'\mathbf{X}\mathbf{b} = \mathbf{X}'(\mathbf{y}-\mathbf{X}\mathbf{b}) = \mathbf{X}'\mathbf{e} = \mathbf{0}
\end{align}
$$ 左辺$\mathbf{X}'\mathbf{e}$は，$\mathbf{X}$の各列（切片項の$\mathbf{1}$と説明変数$\mathbf{x}$）と残差$\mathbf{e}$との**内積**を並べたものである．それがゼロであるとは，第4章で導入した意味で，**残差がすべての説明変数と直交する**ということにほかならない．

つまり最小二乗法とは，残差平方和という計算問題である以前に，

> **残差が説明変数と直交するように係数を選ぶ**

という幾何的な操作である．$\mathbf{y}$を説明変数の張る空間へ真上から落とし（正射影），その影を予測値$\mathbf{X}\mathbf{b}$とする．真上から落とすからこそ，落とした先とのズレである$\mathbf{e}$が空間と直角になる．点と平面の距離を最短にするには垂線を下ろす，という初等幾何と同じことを行っている．
...
つまり，$y$の分散成分は平均から回帰直線までのばらつきである$S_R$と回帰直線から$y$までのばらつきである$S_e$に分解できる．

この分解は，先に見た直交から理解できる．残差$\mathbf{e}$は説明変数と直交していたから，平均からのずれ$\mathbf{y}-\bar{y}\mathbf{1}$を斜辺とし，回帰による部分と残差を二辺とする直角三角形ができる．$S_{yy} = S_R + S_e$は，その**三平方の定理**にほかならない．各項が平方和（＝ベクトルの長さの二乗）であることに注意せよ．

$y$のばらつきを回帰直線だけで表現できる程度が大きいほど，回帰直線としてはうれしい．そこで， $$
