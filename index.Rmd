...
第1回〜第3回はオンライン授業（オンデマンド型），第4回以降は対面授業（月曜2限，E-302）である．
...
4. 【コアウィークス①】線形代数の基礎(1)：多変量解析を理解する上で必要不可欠な線形代数を導入する．ベクトルの直交を理解することを目標とする．
5. 【コアウィークス②】線形代数の基礎(2)：引き続き線形代数の知識を導入する．行列演算と逆行列を理解することを目標とする．
6. 【コアウィークス③】単回帰分析(1)：まず，記述統計レベルで単回帰分析を導入する．最小二乗法を説明し，どのように回帰係数，決定係数を導くかを解説する．また，共分散・相関係数との関連を説明する．
7. 【コアウィークス④】単回帰分析(2)：確率モデルとしての単回帰モデルの仮定を説明し，回帰係数の標本分布を導く．
8. 【コアウィークス⑤】単回帰分析(3)：確率モデルとしての単回帰モデルの仮定を説明し，回帰分析における推定・検定について説明する．
9. 【コアウィークス⑥】重回帰分析(1)：記述統計レベルで重回帰分析を導入する．重回帰における最小二乗法を説明し，偏回帰係数，決定係数を導き，その意味について説明する．
10. 【コアウィークス⑦】重回帰分析(2)：確率モデルとしての重回帰分析について解説する．重回帰分析における推定・検定について説明するとともに，多重共線性について解説する．
11. 【コアウィークス⑧】重回帰分析(3)：ダミー変数，交互作用項を導入したモデルについて解説する．
12. 【コアウィークス⑨】ロジスティック回帰分析(1)：線形回帰モデルを拡張した一般化線形モデルのアイデアを導入する．そのうえでベルヌイ分布（二項分布）を仮定するロジスティック回帰を導入する．
13. 【コアウィークス⑩】ロジスティック回帰分析(2)：引き続き，ロジスティック回帰分析について解説する．とくに係数とオッズ比の関連について説明する．
14. 【コアウィークス⑪】分析実習(1)：実習データを元に回帰分析をRで実行する．その際，自ら計算する場合と組み込み関数を使う場合を比較し，出力の読み方を学習する．あわせて最終リポートの分析を開始し，その日の進捗を提出する．
15. 【コアウィークス⑫】分析実習(2)：ひきつづき実習データを元にロジスティック回帰分析をRで実行する．その際，自ら計算する場合と組み込み関数を使う場合を比較し，出力の読み方を学習する．最終リポートを完成させ，確認を受けたうえで提出する．
