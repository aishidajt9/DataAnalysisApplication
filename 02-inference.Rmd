...
-   母集団の何らかの特性について，その分布を**母集団分布**(population distribution)という．母集団が十分に大きい場合，母集団サイズが無限の無限母集団を仮定し，母集団分布は何らかの確率分布$f(x)$であると仮定する．これによって，各々の標本$X_1, X_2, \cdots, X_n$は$f(x)$に従う確率変数であると見なすことができる（確率変数である標本の実現値は普通小文字で$x_1,x_2,\cdots,x_n$と表す）．特に，無作為抽出標本の場合は「標本$X_1, X_2, \cdots, X_n$は，**同一の**母集団分布$f(x)$に従う$n$個の**独立な**確率変数である」ということを仮定してよい．これを**独立同分布**(Independent and Identical Distributed; iid)の仮定という．
