...
-   母集団の何らかの特性について，その分布を**母集団分布**(population distribution)という．母集団が十分に大きい場合，母集団サイズが無限の無限母集団を仮定し，母集団分布は何らかの確率分布$f(x)$であると仮定する．これによって，各々の標本$X_1, X_2, \cdots, X_n$は$f(x)$に従う確率変数であると見なすことができる（確率変数である標本の実現値は普通小文字で$x_1,x_2,\cdots,x_n$と表す）．特に，無作為抽出標本の場合は「標本$X_1, X_2, \cdots, X_n$は，**同一の**母集団分布$f(x)$に従う$n$個の**独立な**確率変数である」ということを仮定してよい．これを**独立同分布**(independent and identical distribution; iid)の仮定という．
...
-   標準正規分布$N(0,1)$から，サイズ$n$の標本を抽出し平均を取る操作を1000回繰り返す．その$t$値の分布と標準正規分布(赤)と$t$分布$t(n-1)$(青)．正規分布以外の分布からの標本抽出でも，標本サイズが十分に大きくなると，$t$分布は標準正規分布に近づくことを確認できる．
...
-   点推定値が，どの程度正確かを示すために，推定量の分布（標本分布）のばらつきを情報として示すことがある．その際，推定量の標準偏差が一般的に用いられる．これを**標準誤差**(standard error)といってs.e.と略す．
...
-   より具体的に区間推定とは，真の母数の値$\theta$が，ある区間$[L,U]$に入る確率を$1-\alpha\ \ (0\leq \alpha \leq 1)$となるように保証する方法であり， \begin{align}
      P(L\leq \theta \leq U)= 1-\alpha
    \end{align} となる確率変数$L,U$の実現値を求めるものである．$1-\alpha$は**信頼係数**(confidence coefficient)と呼ばれ，このとき，区間$[L,U]$を$100(1-\alpha)\%$**信頼区間**(confidence interval)と呼ぶ．
...
# m: 母平均, s: 母標準偏差, n: 標本サイズ, r: 試行回数
CItest<-function(m,s,n,r) {
  rdata<-replicate(r,{
    ex<-rnorm(n,m,s)
    smpl_m<-mean(ex)
    ci<-qt(0.975, df = n-1)*sd(ex)/sqrt(n)
    c(smpl_m-ci,smpl_m,smpl_m+ci)
  })
...
-   例）母集団分布は正規分布であるが母分散は未知であり，標本サイズは16であった．このとき，標本平均が165センチ，不偏標本分散が100センチであることが分析結果から分かった．帰無仮説$H_0: \mu = 160$のもとでの$t$統計量は $$
    \begin{align*}
