...
母集団上で確率変数$Y$が（確率変数ではない）変数$x$と係数$\beta_0, \beta_1$によって，平均が規定される正規分布に従っていると考える．
\begin{align}
...
$$
\begin{align}
\hat{\beta}_1 \sim N(\beta_1,\sigma^2/S_{xx}) \label{dist_beta_hat}
\end{align}
$$
...
\end{align*}
$$
となる．データサイズは$n=100$なので，自由度98の$t$分布より，
\begin{align*}
	P(|T|>13.842) = 2* 10^{-16}
\end{align*}
が$p$値として得られる．
