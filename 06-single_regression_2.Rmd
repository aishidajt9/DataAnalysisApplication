...
p <- tibble(
    ...
    n = 1:100 |> map(\(i) rep(i, 100)) |> list_c()
) |>
  ggplot(aes(x=x, y=y)) +
  geom_point() +
  geom_abline(slope= 1, intercept= 10, col = "red", lwd=1.5) +
...
仮想データのパラメータ（真の係数，誤差の分散，サンプルサイズ）の設定を変えて，$\hat{\beta}_1$の標本分布を求めるシミュレーションを実行せよ．とくにサンプルサイズを変えたとき標本分布の散らばりがどう変わるかを確認すること．Rスクリプトを実行した結果をWordにコンパイルしたファイルをLUNA提出
